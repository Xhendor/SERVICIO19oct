...
router.get('/', function(req, res, next) {
	...
	if(req.session.loggedIn) {
		pm.obtener().then(productos => {
			...
			//Cambiar a mensaje flash enviando a un render
			return res.status(500).send('Error en obtener productos')
		})
	}else {
		req.flash('error', 'Tiene que iniciar sesion primero!')
...
router.post('/insertar', function(req, res, next) {
	...
	if(req.session.loggedIn) {
		...
		pm.insertar(nombre, '', precio).then(resultado => {//'' = descripcion para probar en postman
			//Cambiar a mensaje flash enviado a un render
			//res.json(resultado)
			pm.obtener().then(productos => {
				...
				//Cambiar a mensaje flash enviado a un render
				return res.status(500).send('Error insertando productos')
			})
		}).catch(err => {
