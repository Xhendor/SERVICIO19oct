...
module.exports = {
	...
	actualizar(id, nombre, precio){
		return new Promise((resolve, reject) => {
			conexion.query('update productos set nombre =?, precio=? where id_productos=?', [nombre, precio, id], (err) => {
				if(err)reject(err);
				else(resolve);
			})
		})
	...
	eliminar(id){
		...
		return new Promise(async (resolve, reject) => {
			...
			conexion.query('delete from productos where id_productos =?', [id], (err) => {
				if(err)reject(err);
				else resolve();
			})
		})
	...
	agregarFoto(idProducto, nombreFoto){
		return new Promise((resolve, reject) => {
			conexion.query('insert into fotos_productos (id_producto, foto) values (?,?)', [idProducto, nombreFoto], (err, resultados) => {
				if(err)reject(err);
	...
	obtenerFotos(idProducto){
		return new Promise((resolve, reject) => {
			conexion.query('select id_producto, foto FROM fotos_productos WHERE id_producto = ?', [idProducto], (err, resultados) => {
				if(err)reject(err);
				else resolve(resultados);
			})
		})
	...
	obtenerPorId(id) {
		return new Promise((resolve, reject) => {
			conexion.query('select id_productos, nombre, descripcion, precio from productos where id_productos=?', [id], (err, resultados) => {
				if(err)reject(err);
	...
	obtenerPrimerFoto(idProducto){
		return new Promise((resolve, reject) => {
			conexion.query('select foto from fotos_productos WHERE id_producto = ? limit 1', [idProducto], (err, resultados) => {
				if(err)reject(err);
